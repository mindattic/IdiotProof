...
public class ScriptParserSerializationTests
{
    ...
    #region AdaptiveOrder Round-Trip

    [Test]
    public void RoundTrip_AdaptiveOrderAggressive()
    {
        var originalScript = "Ticker(AAPL).Entry(150).TakeProfit(160).StopLoss(145).AdaptiveOrder(IS.AGGRESSIVE)";
        var strategy = StrategyScriptParser.Parse(originalScript);

        var serialized = StrategyScriptParser.ToScript(strategy);
        var reparsed = StrategyScriptParser.Parse(serialized);

        Assert.That(reparsed.Segments.Any(s => s.Type == SegmentType.AdaptiveOrder), Is.True);
    }
    ...
    {
        var originalScript = "Ticker(CIGL).Session(IS.PREMARKET).GapUp(5).IsAboveVwap().IsDiPositive().Entry(4.15).TakeProfit(4.80).StopLoss(3.90).AdaptiveOrder(IS.AGGRESSIVE)";
        var strategy = StrategyScriptParser.Parse(originalScript);
