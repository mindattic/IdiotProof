    ...
    public static class Backtester
    {
            ...
            BacktestConfig? config = null)
        {
            ...
            foreach (var bar in bars)
            {
                ...
                while (simulator.TryGetCompletedTrade(out var trade))
                {
                    ...
                    if (config.VerboseLogging)
                    {
                        ...
                        System.Console.WriteLine($"  [{trade.ExitTime:MM/dd HH:mm}] {icon} {trade.ExitReason}: ${trade.NetPnL:F2}");
                    }
                }
            }
            ...
            if (simulator.TryCloseOpenPosition(bars[^1], out var finalTrade))
            {
                ...
                if (config.VerboseLogging)
                {
                    System.Console.WriteLine($"  [{finalTrade.ExitTime:MM/dd HH:mm}] ○ End of data: ${finalTrade.NetPnL:F2}");
                }
            }
            ...
            bool verboseLogging = false)
        {
            if (bars.Count < 50)
            {
                System.Console.WriteLine($"[WARN] Need at least 50 bars for autonomous learning, got {bars.Count}");
            ...
            if (verboseLogging)
            {
                System.Console.WriteLine($"[LEARN] Running autonomous learning for {symbol} with {bars.Count} bars...");
            }
            ...
            foreach (var bar in bars)
            {
                ...
                if (verboseLogging)
                {
                    foreach (var trade in trades)
                    {
                        ...
                        System.Console.WriteLine($"  {icon} {trade.EntryTime:MM/dd HH:mm} -> {trade.ExitTime:HH:mm} | " +
            ...
            if (simulator.HasOpenPosition)
            {
                ...
                {
                    System.Console.WriteLine($"  [END] Closed at end of data: ${finalTrade.ProfitLoss:F2}");
                }
            }
            ...
            if (verboseLogging)
            {
                System.Console.WriteLine($"[LEARN] Complete: {profile.GetSummary()}");
            }
            ...
            bool verboseLogging = false)
        {
            ...
            if (verboseLogging)
            {
                System.Console.WriteLine($"[LEARN] Profile saved to disk for {symbol}");
            }
